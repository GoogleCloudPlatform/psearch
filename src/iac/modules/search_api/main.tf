...
resource "google_artifact_registry_repository" "search_api_repo" {
  location      = var.region
...
data "archive_file" "source_zip" {
  type        = "zip"
  source_dir  = "${path.root}/../../src/psearch/serving"
  ...
  provisioner "local-exec" {
    command = <<EOF
      gcloud builds submit ${data.archive_file.source_zip.output_path} \
        ...
        --region=${var.region} \
        --tag=${var.region}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.search_api_repo.repository_id}/${local.service_name}:latest
    EOF
  }

  depends_on = [
    google_artifact_registry_repository.search_api_repo,
...
resource "google_cloud_run_v2_service_iam_policy" "noauth" {
  location    = google_cloud_run_v2_service.search_api_service.location
  name        = google_cloud_run_v2_service.search_api_service.name
