...
resource "google_bigquery_data_transfer_config" "spanner_export_schedule" {
  ...
  params = {
    query = templatefile("${path.module}/../../bigquery_scripts/transform_embed_export_to_spanner.sql", {
      YOUR_PROJECT_ID       = var.project_id
      YOUR_SPANNER_INSTANCE = var.spanner_instance_id
      YOUR_SPANNER_DATABASE = var.spanner_database_id
    })
